...
class MSClsInfer:
    """
    input: folder_path, which has PLAX-2D and A4C-2D npy file
    ...
    def preprocess(self, folder_path):
        ...
        a4c = get_view(folder_path, 'A4C-2D')
        if len(plax)==0 and len(a4c)==0:
...
if __name__=='__main__':
    infer = MSClsInfer(use_gpu=False, return_output=True)
    infer.load_model()
